...
jest.mock('@/lib/prisma', () => ({
  prisma: {
    ...
    auditLog: {
        ...
        // If we don't mock @/lib/audit, it uses the real function which uses the mocked prisma.
    }
  },
}));
...
describe('Lead Actions', () => {
    ...
    describe('logLeadActivity', () => {
         it('should create activity and revalidate path', async () => {
            (auth as jest.Mock).mockResolvedValue(mockSession);
            (prisma.leadActivity.create as jest.Mock).mockResolvedValue({ id: 'act-1' });
    ...
    describe('updateLeadStatus', () => {
        it('should update status, log activity, and audit log', async () => {
            ...
            // 3. Create Audit Log (via logActivity -> prisma.auditLog.create)
